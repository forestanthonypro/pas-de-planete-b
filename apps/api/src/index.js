...
app.get("/api/meta/last-updated", async (_req, res) => {
  try {
    const [co2, plants, species, fires, vegetation, water, electricity, speciesThreatened, pollution, deputies, anGroups, scrutins, worldBenchmarks] = await Promise.all([
...
app.get("/api/settings/legal-content/:key", async (req, res) => {
...
app.post("/api/admin/settings/legal-content", requireAdminSession, async (req, res) => {
...
app.post("/api/deputy-follows", publicWriteLimiter, async (req, res) => {
...
app.get("/api/deputy-follows/confirm", async (req, res) => {
...
app.get("/api/deputy-follows/unsubscribe", async (req, res) => {
  const { token } = req.query;
  if (!token || typeof token !== "string") {
    return res.status(400).json({ error: "Jeton manquant" });
  }
...
app.post("/api/admin/deputy-follows/send-digests", requireIngestToken, async (_req, res) => {
  try {
