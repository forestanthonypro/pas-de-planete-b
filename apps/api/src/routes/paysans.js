...
router.get("/api/paysan-categories", async (req, res) => {
  const { locale } = req.query;
  try {
    const result = await pool.query("SELECT id, name, slug FROM paysan_categories ORDER BY name");
    const rows = await mergeTranslations(result.rows, "paysan_category", locale);
    res.json(rows);
  } catch (err) {
...
router.put("/api/admin/paysan-categories/:id", requireAdminSession, async (req, res) => {
  const { name } = req.body || {};
  if (!name || !name.trim()) {
  ...
  try {
    const result = await pool.query(
      "UPDATE paysan_categories SET name = $1 WHERE id = $2 RETURNING id, name, slug",
