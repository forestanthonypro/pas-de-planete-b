...
export default function DesabonnerSuivi() {
  ...
  useEffect(() => {
    if (!token) return;
    fetch(`${API_URL}/api/deputy-follows/unsubscribe?token=${encodeURIComponent(token)}`)
    ...
    <div style={{ fontFamily: "sans-serif", padding: "2rem", maxWidth: 600, margin: "0 auto", textAlign: "center" }}>
      ...
      {status === "ok" && (
        <>
          ...
          <p>Tu ne recevras plus d&apos;email pour ce suivi.</p>
        </>
      )}
      {status === "error" && (
        <>
