...
// Libellés d'interface pour la chambre — pas stockés en base (voir
// commentaire dans la migration 040) : "lower"/"upper" sont génériques,
// leur traduction dépend du pays affiché.
...
export default function InternationalMembersPage() {
  ...
  const chamberLabels = CHAMBER_LABELS[country] || { lower: "international.chamber_lower", upper: "international.chamber_upper" };
  ...
  return (
    <div style={{ fontFamily: "sans-serif", padding: "2rem", maxWidth: 900, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", gap: "1rem", marginBottom: "1rem", flexWrap: "wrap" }}>
        ...
        <SearchableSelect
          options={[
            { value: "lower", label: t(chamberLabels.lower) },
            { value: "upper", label: t(chamberLabels.upper) },
          ]}
          value={chamberFilter}
              ...
              {pageItems.map((m) => (
                <tr key={m.id}>
                  <th scope="row" style={{ textAlign: "left", padding: 8, fontWeight: 400 }}>
                    <Link href={`/international/${country}/${m.external_id}`}>{m.full_name}</Link>
                  </th>
                  <td style={{ padding: 8 }}>{t(m.chamber === "upper" ? chamberLabels.upper : chamberLabels.lower)}</td>
      ...
      <p style={{ fontSize: 12, color: "var(--color-texte-clair)", marginTop: "1.5rem" }}>
        ...
        <Link href={`/international/${country}/votes`}>{t("international.votes_link")}</Link>
      </p>
    </div>
  );
}
