...
export default function KitCommunicationActionsCountryPage({ countryCode, countryName, locale }) {
  const { t } = useT();

  const htmlEndpoint = `/api/kit-communication-actions/html/${countryCode}?lang=${locale}`;
  const pdfEndpoint = `/api/kit-communication-actions/pdf/${countryCode}?lang=${locale}`;
  const ogImageUrl = `/api/kit-communication-actions/og-image/${countryCode}?lang=${locale}`;
  const canonicalUrl = `${WEB_URL}${locale === "fr" ? "" : "/" + locale}/kit-communication-actions/${countryCode.toLowerCase()}`;

  const pageTitle = `${t("kit.title_actions")} — ${countryName} | Pas de planète B`;
...
export async function getServerSideProps({ params, locale }) {
  ...
  try {
    const res = await fetch(`${API_URL}/api/kit-communication-actions/country/${code}?lang=${locale}`);
