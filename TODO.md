...
- ✅ **`useCallback` sur les fonctions `build*Chart`** — **déjà résolu**, vérifié le 6 août 2026 : les 5 fonctions (`buildEnergyMixChart`, `buildGenerationChart`, `buildVegetationChart`, `buildWaterChart`, `buildStressChart`) sont déjà au niveau module dans `apps/web/pages/pays/[code].js`, hors du composant, avec `t`/`locale` en paramètres explicites — plus aucun avertissement `exhaustive-deps` (confirmé par lint réel). Ce point était resté dans le TODO par erreur après le refactor qui l'avait déjà réglé.
- ✅ **Script de migration automatisé** — **terminé** (6 août 2026). `db/migrate.sh` applique les fichiers `db/migrations/*.sql` non encore appliqués, dans l'ordre, avec une table de suivi (`_migrations`) pour ne jamais rejouer deux fois la même migration. Mode `--baseline` pour poser une base de référence sur une base où les migrations existantes ont déjà été appliquées manuellement (utilisé pour la production, dont les 37 premières migrations avaient été appliquées à la main le 6 août avant que ce script existe). Intégré dans `deploy-production` (`ci.yml`) : chaque déploiement applique désormais automatiquement les nouvelles migrations. Documenté dans `README.md`.
