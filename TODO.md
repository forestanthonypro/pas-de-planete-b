...
- **Correspondance des sénateurs américains (GovTrack)** : correctif déployé le 10 août (appariement exact par `bioguideId` via la table `congress-legislators`, repli automatique sur nom+état si absent) — **colonnes de l'export CSV GovTrack enfin confirmées le 14 août** via le log `[diagnostic]` : `person, state, district, vote, name, party`. La colonne `person` existe bien (une des hypothèses prévues), l'appariement exact devrait donc fonctionner plutôt que de retomber sur le repli nom+état. À confirmer une fois l'ingestion du 14 août terminée (vérifier le taux de correspondance des votes du Sénat).
...
1. **Fiche scrutin US, lien "voir le texte complet"** : correctif écrit et testé le 14 août (`ingest-us-congress.js`) — construit maintenant un vrai lien vers la page du texte de loi sur Congress.gov (`https://www.congress.gov/bill/{congress}th-congress/{type}/{numéro}`) au lieu du lien vers le fichier XML de résultats du vote. Repli sur l'ancien lien si aucun numéro de loi n'est associé au vote (motions procédurales). **En cours de vérification** : ré-ingestion complète lancée en local le 14 août pour confirmer en conditions réelles (voir aussi note ci-dessous sur le diagnostic GovTrack).
