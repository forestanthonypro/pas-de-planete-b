...
- ✅ **Hook partagé `useApiFetch(path, options)`** — **terminé** (5 août 2026). Éliminait la duplication du pattern `loading/error/fetch` répétée dans ~40 pages. Converti page par page, testé (validation JSX + lint réel ESLint 9 + `eslint-plugin-react-hooks` à chaque lot) :
  ...
  - Reste (si des pages existent en dehors de ce périmètre, à vérifier au prochain passage) : ✅ `pays/[code].js` traité et validé le 5 août 2026 (build réel : `Compiled successfully`, seul `/404` échoue — bug distinct, voir plus haut). Bug d'encodage mojibake (UTF-8/CP1252 double-encodé via `Out-File`/`Get-Content` PowerShell) trouvé et corrigé dans ce fichier au passage. Bug d'ordre de déclaration identique à celui d'`energie.js` corrigé (`vegetationCumulativeSummary` dépendait de `summary` déclaré plus bas).
